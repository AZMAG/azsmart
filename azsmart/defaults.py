...
def load_defaults():
    ...
    # broadcast from building types/factors to agents
    # TODO: potentially broadcast additional columns -- remove from buildings
    for t in ['buildings', 'households', 'jobs', 'seasonal_households', 'persons']:
        make_broadcast_injectable('building_factors', t, 'report_sector_name', 'building_type_name')
    ...
    @orca.column('jobs', 'pub_type', cache=True, cache_scope='step')
    def pub_type(jobs, pub_sb_fsl):
        """
        Put the detail public federal/state or public local info on site based pub jobs.
        """
        return broadcast(pub_sb_fsl.pub_type, jobs.magid).fillna('')


    @orca.column('buildings', 'mpa_respop_growth_this_year', cache=True)
    def bldgs__mpa_respop_growth_this_year(buildings, households, year):
        new_reshh = (households.year_added == year)
        s = households.persons[new_reshh].groupby(households.mpa).sum()
        return broadcast(s,buildings.mpa).fillna(0)


    @orca.column('buildings', 'mpa_respop', cache=True)
    def bldgs__mpa_respop(buildings, households):
        s = households.persons.groupby(households.mpa).sum()
        return broadcast(s,buildings.mpa).fillna(0)


    @orca.column('buildings', 'mpa_poptopubl_factor', cache=True)
    def bldgs__mpa_poptopubl_factor(buildings):
        return 10.55*(buildings.mpa_respop < 10000) + 6.56*((buildings.mpa_respop >= 10000) & (buildings.mpa_respop < 100000)) + 7.50*((buildings.mpa_respop >= 100000) & (buildings.mpa_respop < 200000)) + 9.01*((buildings.mpa_respop >= 200000) & (buildings.mpa_respop < 500000)) + 10.76*(buildings.mpa_respop >= 500000)


    @orca.column('buildings', 'mpa_publ_jobs', cache=True)
    def bldgs__mpa_publ_jobs(buildings, jobs):
        publ_jobs = ((jobs.job_class == 'site based') & (jobs.pub_type == 'local'))
        s = jobs.building_id[publ_jobs].groupby(jobs.mpa).size()
        return broadcast(s,buildings.mpa).fillna(0).astype('int64')


    @orca.column('buildings', 'pubfs_jobs', cache=True, cache_scope='step')
    def bldgs__pubfs_jobs(buildings, jobs):
        pubfs_jobs = ((jobs.job_class == 'site based') & (jobs.pub_type == 'federal_state'))
        s = jobs.building_id[pubfs_jobs].groupby(jobs.building_id).size()
        return s.reindex(buildings.index).fillna(0).astype('int64')


    @orca.column('buildings', 'publ_jobs', cache=True, cache_scope='step')
    def bldgs__publ_jobs(buildings, jobs):
        publ_jobs = ((jobs.job_class == 'site based') & (jobs.pub_type == 'local'))
        s = jobs.building_id[publ_jobs].groupby(jobs.building_id).size()
        return s.reindex(buildings.index).fillna(0).astype('int64')


    @orca.column('buildings', 'publ_weights', cache=True, cache_scope='step')
    def bldgs__publ_weights(buildings):
        bldgs_w_publ_jobs = (buildings.publ_jobs > 0)
        vac_pub_bldgs = (((buildings.building_type_name == 'pub') | (buildings.building_type_name == 'publ'))& (buildings.site_based_jobs == 0))
        pub_bldgs_no_pub_jobs_w_vac_job_spaces = (((buildings.building_type_name == 'pub') | (buildings.building_type_name == 'publ')) & (buildings.pubfs_jobs == 0) & (buildings.publ_jobs==0) & (buildings.vac_job_spaces > 0))

        publ_weights = buildings.parcel_id.copy()
        bldg_indx_all = buildings.parcel_id > -100000
        publ_weights[bldg_indx_all] = 0
        publ_weights[bldgs_w_publ_jobs] = (buildings.mpa_respop_growth_this_year[bldgs_w_publ_jobs]/buildings.mpa_poptopubl_factor[bldgs_w_publ_jobs])*(buildings.publ_jobs[bldgs_w_publ_jobs]*1.0/buildings.mpa_publ_jobs[bldgs_w_publ_jobs])
        publ_weights[vac_pub_bldgs] = (buildings.mpa_respop_growth_this_year[vac_pub_bldgs]/buildings.mpa_poptopubl_factor[vac_pub_bldgs])*((buildings.non_residential_sqft[vac_pub_bldgs]/50)*1.0/buildings.mpa_publ_jobs[vac_pub_bldgs])
        publ_weights[pub_bldgs_no_pub_jobs_w_vac_job_spaces] = (buildings.mpa_respop_growth_this_year[pub_bldgs_no_pub_jobs_w_vac_job_spaces]/buildings.mpa_poptopubl_factor[pub_bldgs_no_pub_jobs_w_vac_job_spaces])*(buildings.vac_job_spaces[pub_bldgs_no_pub_jobs_w_vac_job_spaces]*1.0/buildings.mpa_publ_jobs[pub_bldgs_no_pub_jobs_w_vac_job_spaces])
        return publ_weights.reindex(buildings.index).fillna(0)


# load all defaults upon import
load_defaults()
